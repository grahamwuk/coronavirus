...
server <- function(input, output) {
  ...
  output$cases_date <- renderPlot({
    ggplot(data = selected_countries(), 
         mapping = aes(x = date, y = cases, color=country)) +
    geom_line() +
      scale_y_log10(labels=comma)
  })
  output$cases_align <- renderPlot({
    ggplot(data = selected_countries(), 
         mapping = aes(x = days_since_case_threshold, y = cases, color=country)) +
    geom_line() +
      scale_y_log10(labels=comma,limits=c(input$case_threshold,NA))
  })
